...
n1= 37 #int(float(sys.argv[1])) # 132
n2= 24 #int(float(sys.argv[2])) #461
...
model = Sequential()
model.add(Bidirectional(SimpleRNN(n1, input_shape=(memory, n_features), return_sequences=True)))
model.add(Bidirectional(SimpleRNN(n2, return_sequences=True)))
model.add(Flatten())
