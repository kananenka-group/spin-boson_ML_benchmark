...
n1= 14 #int(float(sys.argv[1])) # 132
n2= 25 #int(float(sys.argv[2])) #461
...
model = Sequential()
model.add(Bidirectional(GRU(n1, input_shape=(memory, n_features), return_sequences=True)))
model.add(Bidirectional(GRU(n2, return_sequences=True)))
model.add(Flatten())
